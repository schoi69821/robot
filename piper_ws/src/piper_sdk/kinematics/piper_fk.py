...
class C_PiperForwardKinematics():
    def __init__(self):
        ...
        self.DH = list(zip(self._a, self._alpha, self._theta, self._d))
...
if __name__ == "__main__":
    # C_PiperForwardKinematics 클래스의 인스턴스를 생성
    piper_fk = C_PiperForwardKinematics()

    # DH 파라미터 출력
    print("DH 파라미터:")
    print(piper_fk.DH)

    # 각 조인트의 각도를 0으로 설정한 배열 (예시)
    joint_angle = [0, 0, 0, 0, 0, 0]

    # 주어진 joint_angle에 대해 Forward Kinematics 계산 수행
    print("\nForward Kinematics 계산 결과:")
    fk_result = piper_fk.CalFK(joint_angle)

    # FK 계산 결과 출력
    for i, result in enumerate(fk_result):
        print(f"프레임 {i + 1}: {result}")
