...
def get_depth_image(observation, camera_names):
    ...
    curr_image = np.stack(curr_images, axis=0)
    curr_image = torch.from_numpy(curr_image / 65535.0).float().cuda().unsqueeze(0)
    return curr_image
...
def model_inference(args, config, ros_operator, save_episode=True):
    ...
    # 发布基础的姿态
    left0 = [-0.005773964, -0.028730268000000003, 0.054983488000000004, -0.04204004000000001, -0.029986236000000003, -0.021002576000000002, 0.80014]
    right0 = [-0.005773964, -0.028730268000000003, 0.054983488000000004, -0.04204004000000001, -0.029986236000000003, -0.021002576000000002, 0.80014]
    left1 = [-0.005773964, -0.028730268000000003, 0.054983488000000004, -0.04204004000000001, -0.029986236000000003, -0.021002576000000002, 0.00014]
    right1 = [-0.005773964, -0.028730268000000003, 0.054983488000000004, -0.04204004000000001, -0.029986236000000003, -0.021002576000000002, 0.00014]
            ...
            while t < max_publish_step and not rospy.is_shutdown():
                ...
                if config['policy_class'] == "ACT":
                    ...
                    if config['temporal_agg']:
                        ...
                        # NOTE: This zero-check heuristic may incorrectly filter out rows where
                        # legitimate action values are all zero. A more robust approach would
                        # track populated indices explicitly, but this is the standard ALOHA
                        # temporal aggregation pattern and changing it may break the algorithm.
...
def get_arguments():
    ...
    parser.add_argument('--temporal_agg', action='store_true', help='temporal_agg')
    ...
    parser.add_argument('--use_actions_interpolation', action='store_true', help='use_actions_interpolation')
